...
select region,round(avg(claim),2) as avg_claim from insurance_data
group by region;
...
select PatientID , claim from insurance_data order by claim desc limit 3;
...
select * , rank() over(order by claim desc) as rank_no from insurance_data;
